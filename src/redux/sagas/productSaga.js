  ...
  switch (type) {
    ...
    case ADD_PRODUCT: {
      try {
        ...
        const key = yield call(api.generateKey);
        const downloadURL = yield call(api.storeImage, key, 'products', payload.image);
        const image = { id: key, url: downloadURL };
        let images = [];

        if (imageCollection.length !== 0) {
          const imageKeys = yield all(imageCollection.map(() => api.generateKey));
          const imageUrls = yield all(imageCollection.map((img, i) => api.storeImage(imageKeys[i](), 'products', img.file)));
        ...
        yield call(api.addProduct, key, product);
    ...
    case EDIT_PRODUCT: {
      try {
        ...
        if (image.constructor === File && typeof image === 'object') {
          ...
          const url = yield call(api.storeImage, payload.id, 'products', image);
        ...
        if (imageCollection.length > 1) {
          ...
          const imageKeys = yield all(newUploads.map(() => api.generateKey));
          const imageUrls = yield all(newUploads.map((img, i) => api.storeImage(imageKeys[i](), 'products', img.file)));
