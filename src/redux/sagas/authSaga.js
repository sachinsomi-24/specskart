...
function* authSaga({ type, payload }) {
  switch (type) {
    ...
    case SIGNIN_WITH_GOOGLE:
      try {
        yield initRequest();
        yield call(api.signInWithGoogle);
      } catch (e) {
        yield handleError(e);
      }
      break;
    case SIGNIN_WITH_FACEBOOK:
      try {
        yield initRequest();
        yield call(api.signInWithFacebook);
      } catch (e) {
        yield handleError(e);
      }
      break;
    case SIGNIN_WITH_GITHUB:
      try {
        yield initRequest();
        yield call(api.signInWithGithub);
      } catch (e) {
        yield handleError(e);
      }
      break;
    case SIGNUP:
      try {
        ...
        const ref = yield call(api.createAccount, payload.email, payload.password);
        ...
        yield call(api.addUser, ref.user.uid, user);
    ...
    case ON_AUTHSTATE_SUCCESS: {
      const snapshot = yield call(api.getUser, payload.uid);
      ...
      if (snapshot.data()) { // if user exists in database
        ...
        yield call(api.addUser, payload.uid, user);
        yield put(setProfile(user));
        yield put(signInSuccess({
          id: payload.uid,
          role: user.role,
    ...
    case SET_AUTH_PERSISTENCE: {
      try {
        yield call(api.setAuthPersistence);
      } catch (e) {
        console.log(e);
      }
      break;
